...
export class AuthService {
  ...
  SetUserData(user, status: string) {
    this.afs.collection('users').get().subscribe((value => {
      let users = value.docs.map(v => v.data());
      let currentRoundStatusNum = users.filter(b => b.roles.status === status).map(account => account.currentRound);
      let currentRound = Math.max(...currentRoundStatusNum);
      ...
      const userData: User = {
        ...
        lastMifgafTime: firestore.Timestamp.now(),
        nextMifgafTime: firestore.Timestamp.now(),
        roles: {
          guest: true,
          status: status
        }
      };
      userRef.get().subscribe(newUser => {
        if (!newUser.exists) {
